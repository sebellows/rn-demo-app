...
describe('createVariant', () => {
  ...
  it('supports referencing other theme values in the variant', () => {
    const variant = createVariant({ themeKey: 'textVariants' })
    expect(variant.func({ variant: 'subheader' }, { theme, dimensions })).toStrictEqual({
      fontSize: 16,
      color: '#111111',
    })
  })
  ...
  it('supports responsive values', () => {
    const variant = createVariant({ themeKey: 'textVariants' })
    expect(
      variant.func({ variant: 'header' }, { theme, dimensions: { width: 768, height: 300 } }),
    ).toStrictEqual({
      fontSize: 28,
      margin: 8,
      fontWeight: 'bold',
  ...
  it('supports more complex responsive values', () => {
    const variant = createVariant({ themeKey: 'textVariants' })
    expect(
      variant.func({ variant: 'header' }, { theme, dimensions: { width: 768, height: 1024 } }),
    ).toStrictEqual({
      fontSize: 34,
      margin: 8,
      fontWeight: 'bold',
  ...
  it('falls back to the closest width', () => {
    const variant = createVariant({ themeKey: 'textVariants' })
    expect(
      variant.func({ variant: 'header' }, { theme, dimensions: { width: 375, height: 1024 } }),
    ).toStrictEqual({
      fontSize: 22,
      margin: 8,
      fontWeight: 'bold',
